...
export default function BussinessHeader() {
        ...
        <Navbar className='white-card' expand="md" style={{ width: "100%" }}>
            ...
            <Nav className="ml-auto " navbar>
              ...
              <NavItem>
                <NavLink>
                  <div>
                    <Button
                      sx={{ padding: 0, margin: 0, color: "black", textTransform: "none", fontWeight: 600 }}
                      id="basic-button"
                    ...
                    >
                      <Link to={paths.businessAgent.editProfile} className='link-text link'><MenuItem onClick={handleClose}>Edit Profile</MenuItem></Link>

                      <Link to={paths.businessAgent.rideDetails} className='link-text link'><MenuItem onClick={handleClose}>My Ride</MenuItem></Link>

                      {/* <Link to={paths.driver.drivehicle} className='link-text link'><MenuItem onClick={handleClose}>My Vehicle</MenuItem></Link> */}

                      <Link to={paths.businessAgent.taxValues} className='link-text link'><MenuItem onClick={handleClose}>Tax Report</MenuItem></Link>

                      <Link to={paths.newsAndUpdate} className='link-text link'><MenuItem onClick={handleClose}>Reviews</MenuItem></Link>

                      <Link to={paths.businessAgent.termsCondition} className='link-text link'><MenuItem onClick={handleClose}>Terms & Condition</MenuItem></Link>
                    </Menu>
                  </div>
                </NavLink>
              </NavItem>
          ...
          <div className='d-flex align-items-center flex-wrap'>
            ...
            {userName ? <span className="link pointer text-primary" > {userName}</span>
              : <Link to={paths.accounts.login} className="btn btn-outline link px-2" sx={{ padding: "8px", color: "#0E4672" }}>  LogIn</Link>}

            {email && <span className="link pointer text-primary" > {email}</span>}

            {!userName && !email ? <button onClick={handleRegistration} className='btn-primary btn-small'>Registration</button>
              :
              <button onClick={handleLogout} className='btn-primary btn-small'>Logout</button>}
          ...
          <div className='py-2'>
            <TranslateDropdown style={{ width: "2rem" }} />
          </div>
        </Navbar>
